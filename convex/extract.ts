...
The final "urgency" value you output MUST be exactly one of "mandatory", "optional", or "informational" — never any other word, even if the document's own label uses different wording. Some "Action Indicator" fields are a full sentence rather than a clean label (e.g. "Recommended to use CLABE to send Payments to Mexico") — map whatever the document's own wording implies to the closest of the three: "recommended"/"suggested"/similar → "optional"; "required"/"must"/"mandatory" → "mandatory"; "no action"/"FYI"/"for your information" → "informational".
